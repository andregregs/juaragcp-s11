...
read -p "${YELLOW}${BOLD}Masukkan REGION: ${RESET}" REGION
read -p "${YELLOW}${BOLD}Masukkan Processor Name di TASK 2: ${RESET}" PROCESSOR
...
while [ "$deploy_success" = false ]; do
  if deploy_function; then
    echo " Sukses Deploy Cloud Run Function"
    deploy_success=true
  else
    echo "Gagal Deploy Cloud Run Function, coba lagi dalam 10 detik"
    sleep 10
  fi
done
