...
# Step 2: Create the job configuration JSON file
    ...
    "inspectJob": {
      ...
      "storageConfig": {
        "cloudStorageOptions": {
          ...
          "fileTypes": [
            "TEXT_FILE", "IMAGE", "WORD", "PDF", "AVRO", "CSV", "TSV", "EXCEL", "POWERPOINT"
          ],
          "fileSet": {
            "regexFileSet": {
...
# Step 3: Send job configuration to DLP API
curl -s \
...
# Step 4: Wait for 15 seconds before activating the job trigger
sleep 15

# Step 5: Activate the job trigger
curl --request POST \
  -H "Content-Type: application/json" \
